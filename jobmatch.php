...
if ($job_id==0) { header('Location: '.APPURL.'job.php'); exit(); }
...
if ($row['job_status']==0) echo '<div class="error">This job is no longer active! <a href="<?php echo APPURL ?>job.php">Search again</a></div>';
else {
...
//    echo '<pre>'.print_r($empA,true).'</pre>';
    ...
    require_once(DOC_ROOT.'/app/views/JobMatchJobView.php');

    echo '<div class="form_row" style="margin:10px 25%; width:50%; border-bottom:dotted 2px #AAA;"></div>';

    require_once(DOC_ROOT.'/app/views/JobMatchResultsView.php');
}
